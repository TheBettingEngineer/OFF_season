...
def load_poland_model():
    # Load data
    pl_df = pd.read_csv("data/POL.csv")
    elo_df = pd.read_csv("data/rating_POL.csv")

    # Team replacements
    pl_df["Home"] = pl_df["Home"].replace("Stal Mielec", "Arka Gdynia")
    pl_df["Away"] = pl_df["Away"].replace("Slask Wroclaw", "Termalica B-B.")
    pl_df["Away"] = pl_df["Away"].replace("Puszcza", "Wisla Plock")
    pl_df["Home"] = pl_df["Home"].replace("Stal Mielec", "Arka Gdynia")
    pl_df["Away"] = pl_df["Away"].replace("Slask Wroclaw", "Termalica B-B.")
    pl_df["Away"] = pl_df["Away"].replace("Puszcza", "Wisla Plock")
    # Team categories
