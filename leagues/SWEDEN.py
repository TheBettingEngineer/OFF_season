...
@st.cache_resource
def load_sweden_model():
    # Load data
    pl_df = pd.read_csv("data/SWE.csv")
    elo_df = pd.read_csv("data/rating_SWE.csv")

    # Team replacements
    pl_df["Home"] = pl_df["Home"].replace("Kalmar", "Degerfors")
    pl_df["Away"] = pl_df["Away"].replace("Kalmar", "Degerfors")
    pl_df["Home"] = pl_df["Home"].replace("Vasteras SK", "Oster")
    pl_df["Away"] = pl_df["Away"].replace("Vasteras SK", "Oster")
    ...
    match_model = joblib.load("models/match_goal_models_sweden.pkl")
    ...
    for _, row in pl_df.iterrows():
        ...
        home_rows_ext.append({
            ...
            "OppCatConcedeVsTeam": get_safe(opp_cat_concede_vs_team, "Team", home, "OpponentCategory", ac,
                                            "OppCatConcedeVsTeam"),
        ...
        away_rows_ext.append({
            ...
            "OppCatConcedeVsTeam": get_safe(opp_cat_concede_vs_team, "Team", away, "OpponentCategory", hc,
                                            "OppCatConcedeVsTeam"),
    ...
    def goal_probabilities_df(home_team, away_team):
        ...
        return pd.DataFrame([
             ...
             **{label: format_percent(home_models_ext[label].predict_proba(X_home)[0][1]) for label in
                ["1+", "2+", "3+"]}},
            {"Team": away_team, "Side": "Away",
             **{label: format_percent(away_models_ext[label].predict_proba(X_away)[0][1]) for label in
                ["1+", "2+", "3+"]}}
        ])
